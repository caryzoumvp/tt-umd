...
bool read_debug_payload_locked(
    ...
    if (!ok) {
        ...
        dbg_log("%s: failed%s", log_prefix, zero_fill_on_failure ? ", zero-filled" : "");
        return false;
    }

    if (log_dump_on_success) {
        dbg_log("%s: ok size=%u dump=%s", log_prefix, size, dump_bytes(static_cast<const uint8_t*>(p), size).c_str());
    }
    return true;
}
...
bool write_debug_payload_locked(
    ...
    send_cmd_locked(
        ...
        size,
        nullptr);
    dbg_log("%s: submitted", log_prefix);
    return true;
}
...
void libttsim_tile_rd_bytes(uint32_t x, uint32_t y, uint64_t addr, void* p, uint32_t size) {
    ...
    std::lock_guard<std::mutex> lock(g_client_mutex);
    dbg_log("rd: tile=(%u,%u) addr=0x%llx size=%u", x, y, static_cast<unsigned long long>(addr), size);
...
void libttsim_tile_wr_bytes(uint32_t x, uint32_t y, uint64_t addr, const void* p, uint32_t size) {
    ...
    std::lock_guard<std::mutex> lock(g_client_mutex);
    dbg_log("wr: tile=(%u,%u) addr=0x%llx size=%u", x, y, static_cast<unsigned long long>(addr), size);
...
void libttsim_dram_rd_bytes(uint32_t x, uint32_t y, uint64_t addr, void* p, uint32_t size) {
    ...
    std::lock_guard<std::mutex> lock(g_client_mutex);
    dbg_log("dram rd: tile=(%u,%u) addr=0x%llx size=%u", x, y, static_cast<unsigned long long>(addr), size);
...
void libttsim_dram_wr_bytes(uint32_t x, uint32_t y, uint64_t addr, const void* p, uint32_t size) {
    ...
    std::lock_guard<std::mutex> lock(g_client_mutex);
    dbg_log("dram wr: tile=(%u,%u) addr=0x%llx size=%u", x, y, static_cast<unsigned long long>(addr), size);
