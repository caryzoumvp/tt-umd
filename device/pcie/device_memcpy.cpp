...
#if defined(__AVX2__)
__attribute__((target("avx2"))) static void memcpy_to_device_avx2(
    std::uint8_t*& d, const std::uint8_t*& s, std::size_t& size) {
    while (size >= 256) {
...
__attribute__((target("avx2"))) static void memcpy_from_device_avx2(
    std::uint8_t*& d, const std::uint8_t*& s, std::size_t& size) {
    while (size >= 256) {
        __m256i v0 = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(s));
        __m256i v1 = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(s + 32));
...
static void memcpy_simd_sse2(std::uint8_t*& d, const std::uint8_t*& s, std::size_t& size) {
    while (size >= 16) {
...
void memcpy_to_device(volatile void* dest, const void* src, std::size_t size) {
...
#if defined(__x86_64__) || defined(_M_X64)
    // The SIMD intrinsics below are not themselves volatile, so the compiler is free to
    ...
    // Phase 1+2: Bulk 256-byte and 32-byte blocks.
#if defined(__AVX2__)
    if (__builtin_cpu_supports("avx2")) {
        memcpy_to_device_avx2(d_simd, s, size);
    }
#endif

    // Phase 3: Remaining 16-byte chunks (SSE2).
    memcpy_simd_sse2(d_simd, s, size);
...
void memcpy_from_device(void* dest, const volatile void* src, std::size_t size) {
    ...
    // in Phases 0/4/5 bound the reordering window for this function.
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast).
    const std::uint8_t* s_simd = const_cast<const std::uint8_t*>(static_cast<const volatile std::uint8_t*>(s));

    // Phase 1+2: Bulk 256-byte and 32-byte blocks.
#if defined(__AVX2__)
    if (__builtin_cpu_supports("avx2")) {
        memcpy_from_device_avx2(d, s_simd, size);
    }
#endif

    // Phase 3: Remaining 16-byte chunks (SSE2).
    memcpy_simd_sse2(d, s_simd, size);
